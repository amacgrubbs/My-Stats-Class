...
####The corpus will need to be cleaned up in order to produce the best models. This clean up included setting all words to lowercase for easier comparision, excluding numbers, punctuation, and whitespace and removing stop words. Stop words are words which do not contain important signifigance to be used in Search Queries. The words filtered in this example are from the SMART information retrieval system.
...
####The end result is a document term matrix of the 2500 documents and the 1389 terms that occur in at least 2.5 percent of the documents. Terms that occur in less than 2.5 percent of the documents are considered sparse and have been removed.
...
NB_table = as.data.frame(table(NB_pred,test_labels))
```

####If we plot the frequency of the predicted classes compared to the actual classes of each document, we can see that the Naive-Bayes model appears to over classify documents with specific authors. For instance, we can see that the model frequently classified documents as being authored by Lydia Zajc, David Lawder, and Alan Crosby.

```{r,echo=FALSE}
plot = ggplot(NB_table)
plot + geom_tile(aes(x=test_labels, y=NB_pred, fill=Freq)) + scale_x_discrete(name="Actual Class") + scale_y_discrete(name="Predicted Class") + theme(axis.text.x = element_text(angle = 90, hjust = 1))
```

####This same information can be seen in the following table which shows the model's sensitivity, the proportion of positive classifications that were labeled as such, and the model's specificity, the proportion of negative classifications taht were labeled as such. Here we see for authors such as Lydia Zajc, David Lawder, and Alan Crosby, there is low specificifty, signifying that the model incorrectly labeled documents as being associated with that author frequently. The sensitivity of the model is also low, as only Lydia Zajc's works were identified correctly with a rate over 90 percent. Combined with the low specificity, we can conclude the model over-classified documents as being authored by her and had difficulty distinguishing her work from the other authors.

```{r,echo=FALSE}
NB_conf = confusionMatrix(table(NB_pred,test_labels))
...
####Before we create this model, we need to add empty columns in the test data set for words that appear in the training data, but not in the test data. This needs to be accomplished as RandomForest requires the same variables in both the training and test data sets, unlike Naive-Bayes.
...
RF_model = randomForest(x=DTM_train_df, y=as.factor(train_labels), mtry=3, ntree=200)
RF_pred = predict(RF_model, data=DTM_test_new)

RF_table = as.data.frame(table(RF_pred,test_labels))
```

####If we plot the frequency of the predicted classes compared to the actual classes of each document, we can see that the random forest model was a much stronger predictor than the Naive-Bayes. The light blue diagonal line signifies an increased frequency of the model classify the documents to the correct author. We do not see the same problem were the Naive-Bayes too frequently classified works as belonging to specific authors due to its inability to distinguish those authors. 

```{r,echo=FALSE}
plot = ggplot(RF_table)
plot + geom_tile(aes(x=test_labels, y=RF_pred, fill=Freq)) + scale_x_discrete(name="Actual Class") + scale_y_discrete(name="Predicted Class") + theme(axis.text.x = element_text(angle = 90, hjust = 1))
```

####We see that the random forest model produced a sensitivity rate of over 90 percent for 10 authors and the specificity rate was over 98 percent for all the authors. These are much more accurate results than the Naive-Bayes model, as it accurately predicted all the authors for at least some of the documents. The Naive-Bayes model failed to classify any documents as belonging to 16 authors, causing the sensitivity rating to be 0. We still see the Random Forest model having difficulty with a few authors, such as Samuel Perry and William Kazer. We can conclude that those authors did not write in a mannor that allowed the model to easily distinguish their writing style from other authors.

```{r,echo=FALSE}
RF_conf = confusionMatrix(table(RF_pred,test_labels))
